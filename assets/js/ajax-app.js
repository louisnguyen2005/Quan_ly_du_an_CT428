...
(function () {
    ...
    function appendCsrfToUrl(url) {
        ...
        if (!parsed.searchParams.has('_csrf_token')) {
            parsed.searchParams.set('_csrf_token', token);
        }
    ...
    window.fetch = function (resource, options) {
        ...
        const parsed = requestUrl ? new URL(requestUrl, window.location.href) : null;
    ...
    document.addEventListener('click', async function (event) {
        ...
        getActionAjax(appendCsrfToUrl(link.href));
    });

    document.addEventListener('DOMContentLoaded', function () {
        document.querySelectorAll('form').forEach(ensureFormCsrf);
    });

    window.csrfToken = csrfToken;
    window.ensureFormCsrf = ensureFormCsrf;
