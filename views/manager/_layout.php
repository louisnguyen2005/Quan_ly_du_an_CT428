...
function mstatus($s){ return match($s){'Pending'=>'Chưa bắt đầu','In Progress'=>'Đang thực hiện','Review'=>'Đang review','Completed'=>'Hoàn thành','Planning'=>'Đang thực hiện', default=>$s ?: 'Chưa rõ'}; }
...
function manager_owned_project_condition(string $alias = 'p', string $param = ':mid'): string {
    return "($alias.created_by = $param OR $alias.manager_id = $param OR EXISTS(
        SELECT 1
        FROM project_members pmx
        WHERE pmx.project_id = $alias.project_id
          AND pmx.user_id = $param
...
function manager_sidebar_counts(PDO $pdo, int $managerId): array {
    ...
    $projects->execute(['mid' => $managerId]);
...
function manager_sidebar(array $manager, string $active='overview', int $projectCount=0, int $taskCount=0, int $notifCount=0): void { ?>
