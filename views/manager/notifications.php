...
// Mở trang "Thông báo" coi như đã xem hết - đánh dấu tất cả thông báo chưa đọc thành đã đọc
// TRƯỚC KHI tính lại sidebarCounts, để badge đỏ ở sidebar + nút chuông trên topbar biến mất
// ngay trong lần render này, cho tới khi có thông báo mới thì đếm lại từ đầu.
$pdo->prepare("UPDATE notifications SET is_read = 1 WHERE user_id = ? AND is_read = 0")->execute([$managerId]);
...
.manager-notification-card.no-link{
    cursor:default;
}
.manager-notification-card.no-link:hover{
    transform:none;
    box-shadow:0 8px 24px rgba(15,23,42,.05);
}
                ...
                <?php else: ?>
                    <?php foreach ($notifications as $item): ?>
                        <?php
                            ...
                            // Bấm vào thông báo là nhảy thẳng tới đúng chỗ: "task chờ duyệt" ->
                            // hàng chờ duyệt; task/comment (nhiệm vụ mới, bình luận, báo cáo...) ->
                            // mở luôn chi tiết nhiệm vụ đó. Thông báo hệ thống thì không có link.
                            $relatedId = (int)($item['related_id'] ?? 0);
                            $notifLink = null;
                            if ($nType === 'task_approval' && $relatedId > 0) {
                                $notifLink = 'index.php?page=manager-pending-approval';
                            } elseif (in_array($nType, ['task', 'comment'], true) && $relatedId > 0) {
                                $notifLink = 'index.php?page=tasks&task_id=' . $relatedId;
                            }
                        ?>
                        <a href="<?= $notifLink ? mh($notifLink) : 'javascript:void(0)' ?>" class="manager-notification-card <?= ((int)($item['is_read'] ?? 0) === 0) ? 'unread' : '' ?> <?= $notifLink ? '' : 'no-link' ?>" data-type="<?= mh($nType) ?>">
